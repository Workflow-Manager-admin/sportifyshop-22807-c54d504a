...
def find_nginx_conf():
    """Locate nginx config files in backend root & analyze for upstream settings."""
    confs = []
...
if __name__ == '__main__':
    ...
    print("\n==== Nginx Configurations (if any) in Backend ====")
    confs = find_nginx_conf()
    print(confs if confs else "No nginx config files found in backend directory.")
    if confs:
        for cf in confs:
            print(f"--- {cf} ---")
            try:
